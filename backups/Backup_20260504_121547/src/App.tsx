...
export default function App() {
  ...
  const handleContext = useCallback(async (id: string, x: number, y: number) => {
    ...
    setMenu({ x, y, id });
    setMenuMetadata(null); // Reset while loading
     ...
     const handleKeys = (e: KeyboardEvent) => {
       ...
       switch(key) {
         ...
         case 'f':
           ...
           break;
         case 'm': toggleMasterMute(); break; case 'l': setGlobalRepeat(prev => { const modes = ['none', 'once', 'always', 'folder']; const next = modes[(modes.indexOf(prev) + 1) % modes.length]; addLog('Global Repeat: ' + next.toUpperCase()); return next; }); break; case 'p': setShowShare(prev => !prev); break; case 'escape':
            e.preventDefault();
            e.stopPropagation();
